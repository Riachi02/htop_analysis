...
def run_sudo_command(command):
    ...
    if password:
        try:
            ...
            result = subprocess.run(f'echo {password} | sudo -S {command}', shell=True, check=True, text=True, capture_output=True)
...
def open_new_window():
    ...
    new_window.title("Funzionalità Avanzate")
    new_window.geometry("800x600")
    Label(new_window, text="Lista dei processi attivi:").pack(pady=20)
    ...
    print(data)
    t = create_table(new_window, data)
    # Crea un DataFrame
...
def main():
    ...
    # Aggiungi un bottone in basso a destra per aprire una nuova finestra
    open_window_button = Button(root, text="Apri Nuova Finestra", command=open_new_window)
    open_window_button.pack(side=RIGHT, anchor=SE, padx=20, pady=20)
